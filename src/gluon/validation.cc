...
ValidationContinuation validateAndCompress(int *reader,
    ...
    //args.push_back("/Users/anonymized/Source/Repos/gluon/images/iphone.jpg");
    //args.push_back("/Users/anonymized/Source/Repos/gluon/test.lep");
    auto encode_pipes = IOUtil::start_subprocess(args.size(), &args[0], false);
    gluon_data->reserve(4096 * 1024);
    size_t size = 0;
    ...
    if (header.size() <= gluon_data->size()) {
    ...
    int gluon_output_pipes[2] = {-1, -1};
    int gluon_roundtrip_send[2] = {-1, -1};
    int jpeg_roundtrip_recv[2] = {-1, -1};
    //int err;
    while(pipe(jpeg_input_pipes) < 0 && errno == EINTR){}
    while(pipe(gluon_output_pipes) < 0 && errno == EINTR){}
    pid_t encode_pid;
    pid_t decode_pid;
    if ((encode_pid = fork()) == 0) { // could also fork/exec here
        ...
        *writer = gluon_output_pipes[1];
        while(close(jpeg_input_pipes[1]) < 0 && errno == EINTR){}
        while(close(gluon_output_pipes[0]) < 0 && errno == EINTR){}
        return ValidationContinuation::CONTINUE_AS_JPEG;
    }
    while(close(jpeg_input_pipes[0]) < 0 && errno == EINTR){}
    while(close(gluon_output_pipes[1]) < 0 && errno == EINTR){}

    while(pipe(gluon_roundtrip_send) < 0 && errno == EINTR){}
    ...
    if ((decode_pid = fork()) == 0) { // could also fork/exec here
        ...
        *reader = gluon_roundtrip_send[0];
        *writer = jpeg_roundtrip_recv[1];
        while(close(gluon_roundtrip_send[1]) < 0 && errno == EINTR){}
        while(close(jpeg_roundtrip_recv[0]) < 0 && errno == EINTR){}

        return ValidationContinuation::CONTINUE_AS_GLUON;
    }
    while(close(gluon_roundtrip_send[0]) < 0 && errno == EINTR){}
    ...
    if (header.size() <= gluon_data->size()) {
        rtmd5 = IOUtil::send_and_md5_result(
            &(*gluon_data)[header.size()],
            gluon_data->size() - header.size(),
            gluon_roundtrip_send[1],
