                      ...
                      ProbabilityTablesBase &pt)
{
   ...
   int adv_predicted_dc = probability_tables.adv_predict_or_unpredict_dc(context.here().dc(),
    ...
    if ((!g_threaded) && GluonDebug::raw_YCbCr[(int)color]) {
        int16_t outp[64];
        ...
        int debug_width = GluonDebug::getDebugWidth((int)color);
        int offset = k_debug_block[(int)color];
        for (int y = 0; y  < 8; ++y) {
            for (int x = 0; x  < 8; ++x) {
                GluonDebug::raw_YCbCr[(int)color][offset + y * debug_width + x] = std::max(std::min(outp[(y << 3) + x] + 128, 255),0);
            }
        }
